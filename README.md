...
Your private key is read from the folder's `.env`, held in memory by the official
...
`init` creates a new `potd-trader/` folder where you are (`potd-trader init <name>` picks another;
an existing folder is never touched), asks 4 questions in the terminal with the two keys typed
with echo off, writes the folder's `.env` at mode 600, then runs `status` and a dry run so you see
what it would buy. It ends in dry-run mode unless you type the phrase it asks for. No `uv` yet?
...
Every command reads the `.env` in the current directory. The folder holds two files that never
leave it: `.env` and, once a live order exists, `ledger.json`. Its `.gitignore` already lists both.
...
The repository-checkout layout: the `.env` sits next to the code, which is the current directory
...
docker run --rm -v "$PWD:/app/data" potd-trader    # from inside the folder init created
```
...
`ledger.json`, beside `.env`, records every live order with the pick's date, rank, token, stake, and the
...
| `LEDGER_PATH` | `ledger.json` beside `.env` | where orders are recorded |
| `POTD_TRADER_HOME` | `~/.potd-trader` | a machine-wide `.env` used when the current directory has none |
...
- Keep the folder's `.env` private (`init` writes it at mode 600 in a mode 700 folder). Anyone with that file can trade from your account.
