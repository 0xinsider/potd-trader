...
<https://docs.polymarket.com/getting-started/python> (installed pin: `polymarket-client==0.10.0`).
...
3. Never set `LIVE=yes`: never run `potd-trader live on`, never type its confirmation phrase,
   ...
   `--dry-run` is safe regardless of the inherited live value. Fake test fixtures are not live settings.
...
9. Never delete a lock file while a process could use it. All concurrent instances must share
   one configuration folder and one ledger on a local filesystem. Stop old releases before upgrading.
...
uv run --locked potd-trader run --dry-run  # requires a Pro read key, no order submissions
```

A change to `polymarket.py` is verified against the installed SDK's real signatures and a real
book (`PublicClient().estimate_market_price(...)` on a live token), alongside the fake-exchange regression cases. Never submit real orders for verification.
